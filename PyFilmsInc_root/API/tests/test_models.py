...
class TestSeatReserved(TestCase):
    ...
    def setUp(self):
        ...
        reservation = Reservation.objects.create(id=5, screening_id=screening,
                                                 reservation_type="SENIOR", user_id=user)
...
class TestTransaction(TestCase):
    ...
    def setUp(self):
        ...
        user = User.objects.create(id=45, username="user1234", email="user1234@gmail.com",
                                   password="userpassword1234")
        Transaction.objects.create(id=102, date_time=date_time, amount=7.25, user_id=user)
        Transaction.objects.create(id=103, transaction_type="CASH", date_time=date_time,
                                   amount=7.25, user_id=user, successful=True)
    ...
    def test_transaction_has_correct_user(self):
        transaction = Transaction.objects.get(id=102)
        self.assertEqual(transaction.user_id.id, 45, "Transaction is not linked to correct "
                                                     "user_id")
        self.assertEqual(transaction.user_id.username, "user1234", "Transaction is not linked to "
                                                                   "correct user_id")
...
class TestReservation(TestCase):
    ...
    def setUp(self):
        ...
        Reservation.objects.create(id=5, screening_id=screening, reservation_type="SENIOR",
                                   user_id=user)
        Reservation.objects.create(id=6, screening_id=screening, user_id=user)
